...
Resource Allocation Summary:
    V terminals (3): VDD, OUT2, OUT1
        → IV5270B HR SMUs (channels 1-3)
    
    I terminals (10): TRIM2, F11, TRIM1, F12, X2, X1, KGAIN2, KGAIN1, IREFP, IMEAS
        → IV5270B channels 4-8 (5 channels)
        → IV4156B channels 1-4 (4 channels)
        → IMEAS uses sequential measurement with IREFP channel
    
    GNDU terminal (1): VSS
        → IV5270B GNDU
    
    VSU terminals (2): VCC, ERASE_PROG
        → IV4156B VSU21, VSU22

Note: Due to resource constraints (10 I terminals, 9 available SMUs after V 
allocation), IMEAS shares measurement timing with IREFP. The configuration
includes both but sequential measurement is required for these terminals.
"""
...
COMPUTE_TERMINALS = {
    ...
    "TRIM2": TerminalConfig(
        ...
        instrument=InstrumentType.IV5270B,
        channel=4,
        description="Trim current 2 - HR SMU (resource overflow)"
    ),
    "F11": TerminalConfig(
        terminal="F11",
        measurement_type=MeasurementType.I,
        instrument=InstrumentType.IV5270B,
        channel=5,
        description="F11 current measurement - MP SMU"
    ),
    ...
    "X1": TerminalConfig(
        ...
        channel=1,
        description="X1 current measurement - 4156B SMU"
    ),
    ...
    # IMEAS requires sequential measurement - shares timing slot with IREFP
    "IMEAS": TerminalConfig(
        terminal="IMEAS",
        measurement_type=MeasurementType.I,
        instrument=InstrumentType.IV4156B,
        channel=4,  # Shared with IREFP - SEQUENTIAL ONLY
        description="Measurement current - SEQUENTIAL with IREFP"
    ),
    ...
    "VCC": TerminalConfig(
        ...
        instrument=InstrumentType.IV4156B,
        channel=21,
        description="VCC voltage source - 4156B VSU1"
    ),
    "ERASE_PROG": TerminalConfig(
        ...
        instrument=InstrumentType.IV4156B,
        channel=22,
        description="Erase/Program voltage source - 4156B VSU2"
    ),
}
...
# Sequential measurement pairs (terminals sharing resources)
COMPUTE_SEQUENTIAL_PAIRS = [
    ("IREFP", "IMEAS"),  # Share IV4156B channel 4
]
